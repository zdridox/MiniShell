...
/*   Updated: 2026/04/23 22:55:48 by mamelnyk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_redirect_node	*new_redirect;
	t_redirect_node	*current_redirect;
...
t_ast_node	*parse_command(t_token **current_token)
{
	...
	words_head = NULL;
	redirects_head = NULL;
	while ((*current_token)->type == WORD || is_redirect((*current_token)->type))
	{
		if ((*current_token)->type == WORD)
		{
...
t_ast_node	*parse_primary(t_token **current_token)
{
	...
	else if (*current_token && (*current_token)->type == WORD)
	{
...
t_ast_node	*parse_pipeline(t_token **current_token)
{
	...
	if (*current_token && (*current_token)->type == PIPE)
	{
		...
		pipeline_node = create_logical_node(NODE_PIPE, NULL, left, right);
		if (!logical_node)
...
t_ast_node	*parse_logical(t_token **current_token)
{
	...
	if (!left)
		return (NULL);
	if (*current_token && is_logical((*current_token)->type))
	{
		(*current_token) = (*current_token)->next;
		right = parse_pipeline(current_token);
		if (!right)
			return (free_ast(left), NULL);
		node_type = token_type_to_node_type((*current_token)->type);
		logical_node = create_logical_node(node_type, NULL, left, right);
		if (!logical_node)
			return (free_ast(left), free_ast(right), NULL);
		return (logical_node);
	}
